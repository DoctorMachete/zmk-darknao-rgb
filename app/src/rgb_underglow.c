...
// Per-position mask marking which pixels the PERSISTENT layer (overrides +
// relocatable indicators) has painted this frame. Used to composite those
// pixels selectively ON TOP of the live underglow/layer buffer, instead of
// replacing the whole strip (which would black out the layer RGB).
static bool status_overlay_active[STRIP_NUM_PIXELS];
...
static void zmk_led_write_pixels(void) {
    ...
    // fast path: no status indicators, no persistent overlay, battery level OK
    if (blend == 0 && !persist_overlay && bat0 >= 20) {
    ...
    if (persist_overlay) {
        // Persistent overlay mode: start from the live underglow/layer buffer
        // and stamp ONLY the overlay positions on top. This preserves the layer
        // RGB everywhere else (the override pixel does not black out the strip).
        for (int i = 0; i < STRIP_NUM_PIXELS; i++) {
            led_buffer[i] = status_overlay_active[i] ? status_pixels[i] : pixels[i];
        }
    } else if (blend == 0) {
        for (int i = 0; i < STRIP_NUM_PIXELS; i++) {
...
static void persist_paint_battery(void) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (bat_level >= min_level) {
            ...
            status_overlay_active[pos] = true;
        }
    }
#endif // CONFIG_ZMK_BATTERY_REPORTING
}
...
static void persist_paint_usb(void) {
    ...
    status_overlay_active[persist_usb.position] = true;
#endif
}
...
static int zmk_led_generate_status(void) {
    ...
    // Reset the persistent-overlay mask each frame.
    memset(status_overlay_active, 0, sizeof(status_overlay_active));
    ...
    // Relocatable live indicators paint on top of any fixed indicators, and
    // record their positions in the overlay mask.
    ...
    // When the Magic display is active it intentionally takes over the whole
    // strip (full opacity). When it is NOT active, the persistent layer must be
    // composited selectively over the underglow (handled in write_pixels via
    // the overlay mask), so we leave blend at 0 here and let the mask drive it.
    if (state.status_active && blend < 256) {
        // (indicator fade envelope handles its own blend; nothing to do)
    }
